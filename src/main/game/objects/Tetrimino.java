...
public class Tetrimino {
    ...
    public void rotate(int factor) {
        ...
        for (int i = 0; i < rotations[nextDirection].length; i++) {
            ...
            if (!canRotate(node.row, node.col))
                wallKick = true;
        }
    ...
    private void updateRotations(int factor, int[] move) {
        ...
        // Yes there is a more efficient way to do this, but for some reason it works
        // better this way (Looping rotation)
        if (factor == 1) {
            if (this.direction < 3)
                this.direction++;
            else
    ...
    protected boolean canRotate(int nextRow, int nextCol) {
        if (!(nextRow >= 0 && nextRow < MyGame.board.board.length && nextCol >= 0
                && nextCol < MyGame.board.board[0].length))
            return false;

        if (MyGame.board.board[nextRow][nextCol] != null && MyGame.board.board[nextRow][nextCol].id != this.id)
            return false;
